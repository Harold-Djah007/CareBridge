    ...
    <div className={`login-page ${isStaff ? "staff-login" : ""}`}>
      <section className="login-hero">
        ...
        <div>
          ...
          <p className="muted">{isStaff ? "Doctors use the clinic board. Administrators use the command centre for beds, staff, reports, and the audit log." : `Book visits, read your clinical file, pay invoices, and reserve a bed at ${HOSPITAL.name}.`}</p>
          {!isStaff && <div style={{ marginTop: 22, maxWidth: 320 }}><CarePath caption="Your visits, messages, and bed stay in one place" /></div>}
        </div>
      ...
      <section className="login-panel">
        <form className="login-card" onSubmit={submit}>
          ...
          <div className="tabs">
            <button type="button" className={!isStaff ? "active" : ""} onClick={() => { setPortal("patient"); setOpsMode(false); setError(""); }}>Patient</button>
            <button type="button" className={isStaff && !opsMode ? "active" : ""} onClick={() => { setPortal("staff"); setOpsMode(false); setError(""); }}>Clinician</button>
            <button type="button" className={opsMode ? "active" : ""} onClick={() => { setPortal("staff"); setOpsMode(true); setEmail(STAFF.admin.email); setPassword(STAFF.admin.password); setError(""); }}>Operations</button>
          </div>
