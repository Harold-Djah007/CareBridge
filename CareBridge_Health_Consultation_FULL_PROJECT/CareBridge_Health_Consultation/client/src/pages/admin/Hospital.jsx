    ...
    <div className="px-page px-bed-command">
      ...
      <section className="px-bed-map">
        ...
        <div className="px-bed-map-body"><div className="px-bed-bars"><OccupancyBars items={wards.map((ward) => ({ label: ward.name, value: Math.max(0, Number(ward.capacity || 0) - Number(ward.available || 0)), max: Number(ward.capacity || 1) }))} /></div><div className="px-bed-ward-grid">{wards.map((ward) => {
          ...
          return <article className={`px-bed-ward ${tone}`} key={ward.id}><header><span><Building2 size={16} /></span><div><h3>{ward.name}</h3><p>{ward.description || "Hospital ward"}</p></div><button type="button" onClick={() => setEdit({ ...ward })}><Edit3 size={15} /></button></header><div className="px-bed-numbers"><span><strong>{ward.available}</strong><small>free</small></span><span><strong>{used}</strong><small>held / occupied</small></span><span><strong>{ward.capacity}</strong><small>capacity</small></span></div><div className="px-bed-meter"><i style={{ width: `${Math.min(100, ratio)}%` }} /></div><footer>{ratio}% committed</footer></article>;
        })}</div></div>
      </section>
