  ...
  return (
    <div className="px-page px-stock-console">
      ...
      <section className="px-stock-board">
        <header className="px-stock-toolbar"><div><span className="px-kicker">Inventory ledger</span><h2>{visible.length} medicine{visible.length === 1 ? "" : "s"}</h2></div><label><Search size={15} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search SKU, name or category" /></label><div className="px-segmented">{[["all","Active"],["in","In stock"],["out","Out"],["archived","Archived"]].map(([id,label]) => <button key={id} type="button" className={filter === id ? "active" : ""} onClick={() => setFilter(id)}>{label}</button>)}</div></header>
        ...
        <div className="px-stock-list">
          {visible.map((row) => {
            ...
            return <article className={`px-stock-row ${row.archived ? "archived" : low ? "low" : ""}`} key={`${row.id}-${row.qty}-${row.inStock}-${row.archived}`}>
              <div className="px-stock-product"><span className="px-stock-product-icon"><Boxes size={17} /></span><div><input defaultValue={row.name} disabled={busy === row.id} onBlur={(e) => { const name = e.target.value.trim(); if (name && name !== row.name) saveRow(row, { name }); }} /><small>{row.sku || "No SKU"} · {row.form || "Medicine"} · {row.pack || "Pack"}</small></div></div>
              <div><select value={row.category} disabled={busy === row.id} onChange={(e) => saveRow(row, { category: e.target.value })}>{(cats.includes(row.category) ? cats : [row.category, ...cats]).map((category) => <option key={category}>{category}</option>)}</select></div>
              <div><input className="px-stock-number" type="number" min="0" defaultValue={row.price} disabled={busy === row.id} onBlur={(e) => { const price = Number(e.target.value); if (price !== Number(row.price)) saveRow(row, { price }); }} /><small>{ghs(row.price)}</small></div>
              <div><input className="px-stock-number" type="number" min="0" defaultValue={row.qty} disabled={busy === row.id} onBlur={(e) => { const nextQty = Number(e.target.value); if (nextQty !== Number(row.qty)) saveRow(row, { qty: nextQty }); }} />{low && <em>{qty === 0 ? "Empty" : `Low · ${qty}`}</em>}</div>
